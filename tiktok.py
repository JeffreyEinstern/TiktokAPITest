...
MS_TOKEN='xfaycVCvBF69OiMRle6I1dplgqy-K0jGxw6YKrA0FisHEUxS-4tAoO51ubmOW820jW0kP_yuiYILVF7uYAeu2-6uZ20rpXfttCahkC9qjb2gJGmsuY1ZNsDFfy54eQz3qYz3sMkMvmJaWj3o'
...
class TikTokApi:
    ...
    def __init__(
        ...
        * ms_token 接口前面 msToken,X-Bogus,_signature是jsVmp生成。
        msToken 非对称算法生成,和_signature参数相辅相成
        X-Bogus 发现了cookie参与了运算,但是发空值,服务器并未进行检验,后期算法更新估计就会更新这一块
        _signature  检验是否浏览器环境,如果非浏览器生成的算法则发包报错,其中关键点,_signature生成的时候,浏览器环境user-agent参与运算。
    ...
    def get_data(self, path, subdomain="m", **kwargs) -> dict:
        ...
        if kwargs.get("ms_token") == None:
            if self._ms_token != None:
                ...
                try:
                    with open('tiktok.json') as obj:
                        content = json.load(obj)
                        ms_token=content.msToken
                except:
                     ms_token = MS_TOKEN
        else:
        ...
        query = {"device_id": device_id,"msToken":ms_token,"_signature": signature}
        ...
        #保存有效的msToken
        self._ms_token = r.cookies.get('msToken', None)
        if self._ms_token:
          with open('tiktok.json', 'w') as obj:
             json.dump({'msToken': self._ms_token}, obj)
